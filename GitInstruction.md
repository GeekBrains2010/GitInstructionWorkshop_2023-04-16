...
## Работа с созданными файлами и коммитами
 ![Когда объясняешь как работает Git](/image/explain_git.jpg) 

Увидеть изменения которые не закоммичены нам поможет команда
 
    #Показывает различия между проиндексированной и последней зафиксированной версиями файлов
    $ git diff
Так же мы можем в случае необходимости исключить файл из индексации или выполнить другие операции с файлами
...
>В данной статье присутствуют изображения, которые подтягиваются из папки **\images** _Однако добавлять какие либо фотографии либо большие файлы добавлять в **commit в Git** не принято, т.е. сами файлы у нас есть, ссылки на нх тоже есть, но помечать их для добавления в **commit через команду add** не следует. Но в таком случае при вызове комманды **git status** Git будет каждый раз нам сообщать, что у нас есть не омеченные и не закоммиченные файлы.  
Выход из этой ситуации следующий:  
Мы создаем файл **.gitignore** и в нем прописываем названия файлов, изменения в которых мы не хотим отслеживать и добавляем в наш коммит файл **.gitignore**_
